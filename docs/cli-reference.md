...
This document covers ScopeLens configuration, scanner commands, evidence storage,
analysis, reports, and opt-in integration tests. For the supported local application
workflow, start with the [README](../README.md). For deployment, backup, restore, and
the controlled vulnerable/fixed lab, see [local operations and recovery](operations.md).
...
uv run --locked scopelens history-verify
```
...
`history-verify` runs the same artifact checks but exits with failure for missing,
corrupt, busy, or unknown results. Interrupted records and unrelated unreferenced
files remain reported without blocking verification. The restore procedure uses
this command before starting the operational API.
...
pending durable manifest; execution is a separate request and never retries terminal
...
The static public demo consumes a bundled `public-snapshot-v1` fixture generated
from deterministic synthetic acquisitions evaluated through the actual assessment
and comparison code. It has no API client, worker action, token, database, evidence
volume, or scanner binary.
